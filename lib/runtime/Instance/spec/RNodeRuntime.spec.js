...
test('children.indexed should exclude index and underscored files', () => {
    expect(node.children.indexed.length).toBe(1)
    expect(node.children.indexed[0].name).toBe('normal')
})
...
test('children.normal returns a child named normal', () => {
    expect(node.children.normal.name).toBe('normal')
})

test('children should have keys from array, filters and child nodes', () => {
    expect(Reflect.ownKeys(node.children)).toEqual([
        'normal',
        '_underscored',
        '_fallback',
        'index',
