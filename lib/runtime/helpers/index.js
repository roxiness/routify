...
export const url = {
    subscribe: (run, invalidate) => {
        ...
        return derived(router.activeRoute, activeRoute => {
            ...
            return createUrl(router, originNode, activeRoute)
        }).subscribe(run, invalidate)
    },
}
...
export const createUrl =
    ...
    (_inputPath, userParams = {}, options = {}) => {
        const inputIsString = typeof _inputPath === 'string'
        // if we're dealing with an anchor element, get the href
        const inputPath = inputIsString ? _inputPath : _inputPath.getAttribute('href')
        ...
        // @ts-ignore
        if (inputIsString) return externalUrl
        else _inputPath.setAttribute('href', externalUrl)
    }
