...
export const createUrl =
    ...
    (_inputPath, userParams = {}, options = {}) => {
        ...
        const externalUrl = router.getExternalUrl(internalUrl).replace(/\/$/, '') || '/'
        return externalUrl
    }
 ...
 * @param {Route=} route
 */
export const getDirection = (boundary, route) => {
    const _context = get(context)
    const router = _context.router
    route = route || _context.route || router.activeRoute.get()

    boundary = boundary || _context.node.parent
    const { lastRoute } = route?.router || _context.router

    if (!lastRoute) return 'first'

    const mrcaInfo = getMRCA(route.leaf.node, lastRoute.leaf.node)
    ...
    if (oldNode.level > newNode.level) return 'higher'
    if (oldNode.level < newNode.level) return 'lower'
    return 'na'
}
