...
const restoreScrollBehavior = elem => {
    ...
    // todo two adjecent calls to restoreScrollBehavior will leave scrollBehavior undefined, even if it was defined before the backup
...
export const scrollIntoView = async (elem, callback, instant) => {
    ...
    const observer = observeDocument(() => callback(elem), true)
    setTimeout(() => {
        observer.disconnect()
...
export class ScrollIntoViewQueue {
    ...
    async processQueue() {
        ...
        while (this.queue.length && initialId == this.currentId) {
