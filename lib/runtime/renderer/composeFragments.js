...
export const updateVisibility = context => {
    ...
    get(context.childContexts).forEach(_context => {
        _context.isInline = checkIfInline(_context)
        const isBothInlined = _context.isInline && checkIfInline(activeChildContext)
        const envIsOkay = ['always', environment].includes(_context.inline.context)
        const isVisibleAsInlined = isBothInlined && envIsOkay

        _context.wasVisible = get(_context.isVisible)
        lazySet(_context.isActive, _context === activeChildContext)
        lazySet(_context.isVisible, get(_context.isActive) || isVisibleAsInlined)
        // if it's not visible, the element doesn't exist anymore
        if (!get(_context.isVisible)) _context.elem.set(null)
