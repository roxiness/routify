...
import { createHook } from '../../utils/index.js'
...
export class AddressReflector extends BaseReflector {
    ...
    reflect = () => {
        ...
        const addressRouters = routers.filter(
            router => get(router.urlReflector).constructor.name === 'AddressReflector',
        )

        const browserUrl = browserAdapter.toBrowser(addressRouters)

        this.log.debug('pushing internal url to browser', { mode, browserUrl }) // ROUTIFY-DEV-ONLY

        history[`${mode}Native`]({}, '', browserUrl)
    }
}
