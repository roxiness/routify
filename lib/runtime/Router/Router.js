...
export class Router {
    ...
    async _setUrl(url, mode, isInternal, state = {}) {
        ...
        const cachedRoute = cachedRoutes.get(url)
        const currentRoute = this.pendingRoute.get() || this.activeRoute.get()

        if (state.prefetch) {
            if (!cachedRoute) {
                const route = new Route(this, url, mode, state)
                cachedRoutes.set(url, { route, loaded: route.loadRoute() })
            }
        } else {
            const route = cachedRoute?.route || new Route(this, url, mode, state)
            route.mode = mode
