...
export class Router {
    ...
    beforeRender = createHook({ pipeline: true })
    ...
    async _setUrl(url, mode, isInternal) {
        ...
        const oldPendingRoute = pendingRoute.get()
        ...
        debug('current pending route', oldPendingRoute) // ROUTIFY-DEV-ONLY
        if (oldPendingRoute) {
            debug('has pending route') // ROUTIFY-DEV-ONLY
            if (identicalRoutes(oldPendingRoute, route)) {
                debug('current pending route is identical - skip', oldPendingRoute, route) // ROUTIFY-DEV-ONLY
                return true
            }
            // kill current pending route
            oldPendingRoute.invalid = true
            debug('kill existing pending route', route) // ROUTIFY-DEV-ONLY
            await new Promise(resolve => pendingRoute.subscribe(resolve))
        }

        // todo activeRoute belongs to "project"'s parent router
        if (identicalRoutes($activeRoute, route)) {
            debug('new route is identical to active route - abandon', $activeRoute, route)
            return true
        }
        ...
        for (const pretask of pipeline) {
            const pass = await pretask.bind(route)()
            if (!pass || route.invalid) {
                if (pendingRoute.get() === oldPendingRoute) {
                    route.log.debug('pass failed, reset pendingRoute', route) // ROUTIFY-DEV-ONLY
                    // only reset if pending route hasn't changed
        ...
        this.afterUrlChange.hooks.forEach(hook => hook({ route }, _history))

        // unset pending route, if it hasn't changed since we started runBeforeUrlChangeHooks
        if (get(pendingRoute) === route) {
            debug('unset pending route', route) // ROUTIFY-DEV-ONLY
