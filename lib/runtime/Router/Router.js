...
export class Router {
    ...
    async _setUrl(url, mode, isInternal, state = {}) {
        ...
        if (identicalRoutes(currentRoute, route)) {
            ...
            const didLoadRoute = await route.loadRoute()

            // todo move this to router? [START]
            // todo what about multiple pending routes and their resolved promises
