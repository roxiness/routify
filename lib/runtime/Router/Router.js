...
export class Router {
     ...
     */
    constructor({ instance, rootNode, name, routes, urlRewrite } = {}) {
        const parentCmpCtx = getContextMaybe('routify-fragment-context')
        if (urlRewrite) this.urlRewrites.push(...[urlRewrite].flat())
        if (!instance)
            instance =
        ...
        if (routes) {
            const rootNode = instance.createNode().importTree(routes)
            instance.rootNodes[routes.rootName || 'unnamed'] = rootNode
        }
        ...
        this.rootNode = rootNode || Object.values(instance.rootNodes)[0]
