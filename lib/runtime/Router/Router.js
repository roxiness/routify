...
export class Router {
    ...
    constructor(input) {
        ...
        if (matchingRouter) {
            ...
            input.name = (input.name || '').replace(/(\d*)$/, (_, n) => (n ? n + 1 : 1))
        }
        ...
        setTimeout(() => {
            // we're using setTimeout to make sure deleted routers have been destroyed
            this.afterUrlChange(() => setTimeout(() => this._urlReflector.reflect(), 0))
        }, 200)
