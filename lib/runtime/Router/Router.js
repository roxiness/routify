...
export class Router {
    ...
    _setName(name = '') {
        ...
        const { routers } = this.instance.global
        if (routers.find(r => r !== this && r.name === name)) {
            log.debug(`router with name "${name}" is taken.`)
            let count = 0
            let newName = ''
            do {
                log.debug(``)
                count++
                newName = (this.name || 'r') + count
                log.debug(`trying router name: "${newName}".`)
            } while (routers.find(r => r.name === newName))
            this.name = newName
        }
    }
