...
export class Router {
    ...
    constructor(instance, { rootNode, parentCmpCtx, name } = {}) {
        ...
        this.activeRoute = new writable()
        this.pendingRoute = derived(this.activeUrl, $url => new Route(this, $url))
        this.params = derived(this.pendingRoute, $pendingRoute => $pendingRoute.params)

        this.pendingRoute.subscribe(async route => {
    ...
    set url(url) {
        if (url) {
            ...
            this.activeUrl.replace(url) //todo is causing probs
        }
    }
