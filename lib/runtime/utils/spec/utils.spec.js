...
test('populateUrl', () => {
    test('can do root url', () => {
        const url = populateUrl('/', {}, mockRoute.router.queryHandler.stringify)
        assert.equal(url.path + url.query, '/')
    })

    test('can do plain url', () => {
        const url = populateUrl('/foo/bar', {}, mockRoute.router.queryHandler.stringify)
        assert.equal(url.path + url.query, '/foo/bar')
    })

    test('can do url with params', () => {
        const url = populateUrl(
            '/car/[make]',
            { make: 'ford' },
            mockRoute.router.queryHandler.stringify,
        )
        assert.equal(url.path + url.query, '/car/ford')
    })
    ...
    test('can do urls with spread params', () => {
        ...
        assert.equal(url.path + url.query, '/spread/hello/world')
    })
    ...
    test('can do overloads', () => {
        ...
        assert.equal(url.path + url.query, '/overload?str=hello%2Fworld')
    })
})
