...
/** @returns {import('../renderer/types').RenderContext} */
...
/**
 * Takes an array and an A) item to push to the cloned array or B an anonymous function where the first parameter is the cloned array
 * @example
 * ```javascript
 * const array = [1,2,3] *
...
export const pushToOrReplace = (arr, input) => {
    const isAnonFn = typeof input === 'function' && !input.prototype
    input = isAnonFn || Array.isArray(input) ? input : [input]

    const res = isAnonFn ? input([...arr]) : [...arr, ...input]
