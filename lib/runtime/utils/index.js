...
/** @template P
 * @typedef {(pri: P, ...rest)=>(P|void)} Hook
 */
...
/**
 * @template P
 * @typedef {AddHookToCollection<P> & {runHooks: Hook<P>, hooks: Hook<P>[]}} HooksCollection
 */
 ...
 * unhookFn2()
 */
export const createHook = ({ pipeline } = {}) => {
    // todo add test
    ...
    addHook.hooks = hooks
    // todo add parallel and async support
    addHook.runHooks = pipeline
        ? (attr, ...rest) =>
              hooks.length ? hooks.reduce((_attr, cb) => cb(_attr, ...rest), attr) : attr
        : (attr, ...rest) => hooks.forEach(cb => cb(attr, ...rest))
