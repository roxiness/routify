...
export const scrollToTop = (elem, scrollLock) => {
    ...
    while (parent?.scrollTo && scrollLock !== parent) {
...
export class ScrollQueue {
     ...
     */
    async processQueue(route) {
        console.debug('processing scroll queue', this.queue) // ROUTIFY-DEV-ONLY
        this.queue = this.queue.filter(({ scrollCtx }) => scrollCtx.route === route)
    ...
    processRegularQueue(scrollContexts) {
        for (const scrollContext of scrollContexts) {
            console.debug('scroll to top', scrollContext.ctx.node.name, scrollContext) // ROUTIFY-DEV-ONLY
