...
/**
 * @param {RequestInfo & import('node-fetch').RequestInfo} url
 * @param {(RequestInit & import('node-fetch').RequestInit)=} init
 * @returns {Promise<Response | import('node-fetch').Response>}
 */
export const universalFetch = async (url, init) =>
    typeof globalThis.fetch === 'undefined'
        ? import('node-fetch').then(f => f.default(url, init))
        : fetch(url, init)
