...
export class Route {
    ...
    /**
     * converts async component functions to sync functions
     */
    async loadComponents() {
        await Promise.all(
            this.fragments.map(async fragment => {
                const component = await fragment.node.component()
                fragment.node.component = () => component
            }),
        )
    }
    ...
    _createFragments() {
        ...
        const fragments = routeFragments
            .splice(offsetIndex)
            .filter(fragment => fragment.node.component)
