...
export class Route {
    ...
    get queryParams() {
        return this.router.queryHandler.parse(this.search, this)
    }
    ...
    _createUrl(fragments) {
        return (
            populateUrl(fragments.map(f => f.node.name).join('/'), this.params, obj =>
                this.router.queryHandler.stringify(obj, this),
            ).replace(/\/index$/, '') +
            this.search +
            this.hash
        )
