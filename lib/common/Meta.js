...
const metaProxy = {
    ...
    get: function (target, prop, receiver) {
        const basePath =
            target._node?.instance?.options?.routifyDir ||
            target._node?.root?.routifyDir?.replace(/\/[^/]+$/, '') ||
            '.'
        ...
        if (typeof metaProp !== 'undefined')
            return metaProp.split
                ? import(basePath + '/' + metaProp.value).then(r => r.default)
                : metaProp.value
    ...
    set(target, prop, value) {
        ...
        const metaProp = value?.value ? value : { value }
