...
export class RNode {
    ...
    createChild(name, module) {
        const node = /** @type {this} */ (this.instance.createNode(name, module))
     ...
     * @param {boolean=} silent don't throw errors for 404s
     */
    getChainTo(path, allowDynamic = true, includeIndex = true, silent) {
        ...
        while (currentNodeStep.stepsToLeaf.length) {
            ...
            if (nextNode) {
            ...
            } else if (!allowDynamic && silent) return null
            else if (!allowDynamic && !silent)
                throw new Error(
                    `"${nodeSteps
                        .map(ns => ns.node.name)
                ...
                if (!currentNodeStep && silent) return null
                else if (!currentNodeStep && !silent)
