...
export class RNode {
    ...
    constructor(name, module, instance) {
        ...
        this.specificity = {
            static: this.name.replace(/\[.+?\]/g, '').length,
            dynamic: this.paramKeys.length,
        }
        ...
        while (currentNodeStep.stepsToLeaf.length) {
            ...
            const nextNode =
                nextStep === '..'
                    ...
                    : (!inStaticDeadEnd &&
                          currentNodeStep.node.navigableChildren.find(
                              node => node.name === nextStep,
                          )) ||
                      (options.allowDynamic &&
                          !inDynamicDeadEnd &&
                          [...currentNodeStep.node.navigableChildren]
                              .sort(
                                  (a, b) =>
                                      // sort by static specificity, then dynamic specificity
                                      b.specificity.static - a.specificity.static ||
                                      b.specificity.dynamic - a.specificity.dynamic,
                              )
