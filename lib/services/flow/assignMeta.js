...
async function applyMetaToFiles({ tree, metaParser }) {
  return await walkAsync(async function (file) {
    if (file.dir) return
    const meta = await metaParser.get(file.absolutePath)
    file.meta = meta
    if (file.isLayout) {
...
function applyMetaToTree({tree, defaultMeta}) {
