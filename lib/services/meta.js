...
module.exports = ({ cache: useCache } = {}) => {
    ...
    while (fragments = re.exec(content)) {
      if (fragments) {
        ...
        try {
          ...
          console.warn(`[routify] ${fragments[1]} can not be evaluated as a javascript assignment in ${file} (${err.toString()})`)
        }
      }
    }
  ...
  function deleteFile(file) { return useCache && cache.delete(file) || true }
