...
module.exports = async function scanner(options, metaParser) {
  ...
  const treeWithMeta = await applyMetaToFiles(sortedTree, root, metaParser)

  const treeWithAllMeta = applyMetaToTree(treeWithMeta, defaultMeta)
  const svelteTree = removeNonSvelteFiles(treeWithAllMeta)
  const decoratedTree = decorate(svelteTree, options)
...
function attachComponent(tree, options) {
  ...
  return tree.map(file => {
    if (file.dir) file.dir = attachComponent(file.dir, options)
    else {
...
function decorate(tree, options) {
  const { filepathToUrl: _filepathToUrl = filepathToUrl } = options
  return tree.map(file => {
    file.path = _filepathToUrl(
      {
        path: file.filepath,
        extension: file.ext,
    ...
    if (file.dir) {
      file.dir = decorate(file.dir, options)
    } else {
...
function removeNonSvelteFiles(tree) {
  return tree.filter(file => {
    if (file.dir) file.dir = removeNonSvelteFiles(file.dir)
 ...
 */
async function applyMetaToFiles(tree, parent, parser) {
  const treeWithMeta = tree.map(async file => {
    if (file.dir) file.dir = await applyMetaToFiles(file.dir, file, parser)
    else {
...
function applyMetaToTree(tree, dirMeta) {
  return tree.map(file => {
    ...
    if (file.dir) file.dir = applyMetaToTree(file.dir, file.meta)
    return file
  })
}
...
function sortTree(tree) {
  ...
  const sortedTree = sortedFiles.map(file => {
    if (file.dir) file.dir = sortTree(file.dir)
    return file
  })
  return sortedTree
}
