...
module.exports = async function scanner(inputOptions, metaParser) {
  ...
  const buildTree = pipeAsync(
    ...
    walkAsync(
      ...
      defineFiles, // file => ({ isLayout, isReset, isIndex, isFallback, isMagic })
 ...
 *
 * - falsy values for walker will be ignored
 *
...
function walkAsync(...walkers) {
  const actualWalkers = walkers.flat().filter(Boolean)
...
const defineFiles = file => {
  ...
  Object.defineProperty(file, 'isMagic', {
    get() {
      return file.isLayout || file.isReset || file.isIndex || file.isFallback
    },
  })
