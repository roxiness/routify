...
module.exports = async function scanner({ pages, fsa = fsaSingleton }) {
  const tree = await getFileTree(pages, fsa)
 ...
 */
async function getFileTree(absoluteDir, fsa, relativeDir = '') {
  const files = (await fsa.readdir(absoluteDir)).map(async file => {
    const filepath = relativeDir + '/' + file
    const absolutePath = path.resolve(absoluteDir + '/' + file)
    const isDir = (await fsa.stat(absolutePath)).isDirectory()
    const [name, ext] = splitString(file, '.')
    const dir = isDir
      ? await getFileTree(absolutePath, fsa, `${relativeDir}/${file}`)
      : []
    return { file, filepath, name, ext, isDir, dir, relativeDir, absolutePath }
  })
