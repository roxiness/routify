...
const start = function start(inputOptions) {
  // input options take highest precedence, filter out values at their default
  const nonDefaultInputOptions = Object.entries(inputOptions || {}).reduce((options, [key, value]) => {
    if (defaults[key] !== value) {
      options[key] = value
    }
    return options
  }, {})
  const pkgFileOptions = importConfig('package.json','routify');
  const configFileOptions = importConfig(nonDefaultInputOptions.configFile ||defaults.configFile)
...
function importConfig(relativePath,configKey) {
  if(relativePath) {
    const absolutePath = path.resolve(process.env.PWD,relativePath);
    if(fs.existsSync(absolutePath)) {
      try {
        const options = require(absolutePath);
        return configKey ? options[configKey] : options
      } catch (e) {
        log(`failed to import config from ${absolutePath}`, e);
      }
    }
  }
  return {}
