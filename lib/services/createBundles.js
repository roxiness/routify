...
module.exports = function (routes, { pages, outputDir }) {
    ...
    function generateFileTemplates([filename, bundle]) {
        const imports = bundle.map(file => `import ${file.component} from '${pages + file.filepath}'`).join("\r\n")
        const exports = `export  {\n${bundle.map(file => "  " + file.component).join(",\r\n")}\r\n}`
        const body = `${imports} \r\n\r\n ${exports}`
    ...
    function writeFiles(files) {
        files.forEach(({ filename, body }) => filewriter(body, outputDir + '/' + filename))
    }
}