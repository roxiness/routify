...
module.exports = function template(routes, options) {
  ...
  const dynamicImportFiles = dynamicImports ? [] : routes
  ...
  routes.filter(r => r.isLayout)
    .map(keepKeys(['path', 'component', 'options', 'relativeDir']))
    .map(l => ({ ...l }))
    .forEach(layout => { layouts[layout.path] = { ...layout, path: layout.relativeDir } })
  const layoutsTemplate = escapeStuff(stringify(layouts, 0, 2))


  const dynamicImportTemplate = dynamicImportFiles.map(
    route =>
