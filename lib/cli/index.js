...
import * as devServer from '../extra/vite-plugin/devServer.js'
...
program
    ...
    .action(async options => {
        const { RoutifyBuildtime } = await import('../buildtime/RoutifyBuildtime.js')
...
program
    .command('dev')
    .description('Run a Vite dev server for Routify 3')
    .option('-t --template <path>', 'path to HTML template', devServer.defaults.template)
    .option('-e --entrypoint <path>', 'path to entrypoint', devServer.defaults.entrypoint)
    .option('-p --port <number>', 'port for hosting', devServer.defaults.port.toString())
    .action(async options => {
