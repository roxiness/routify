...
export const metaFromFile = async ({ instance }) => {
    ...
    const promises = instance.nodeIndex.map(async node => {
        if (node.file && !node.file.stat.isDirectory()) {
            ...
            const metaPromises = [
                getExternalMeta(node.file.path, context, '.meta.js'),
                getExternalMeta(node.file.path, context, '.meta.ts'),
...
export const getExternalMeta = (filepath, context, ext) => {
    ...
    if (existsSync(metaFilePath)) {
        const time = fse.statSync(metaFilePath).mtimeMs

        const path = './' + relative(__dirname, metaFilePath)
        const cacheBustedPath = `${path}?t=${time}`
        return import(cacheBustedPath).then(r => {
            try {
