...
describe('metaFromFile middleware', () => {
    test('metaFromFile middleware', async () => {
        ...
        expect(await externalMetaNode.meta.codesplitted).toBe(
            "()=> import('./cached/lib/buildtime/plugins/metaFromFile/spec/metaFromFile/example/externalMeta.svelte-codesplitted-split.js').then(r => r.default)::_EVAL",
        )
        expect(await externalMetaNode.meta.explicit).toBe("I'm explicit")
        expect(await externalMetaNode.meta.explicitSplit).toBe(
            "()=> import('./cached/lib/buildtime/plugins/metaFromFile/spec/metaFromFile/example/externalMeta.svelte-explicitSplit-split.js').then(r => r.default)::_EVAL",
        )
