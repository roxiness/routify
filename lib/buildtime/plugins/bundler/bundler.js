import { writeFileIfDifferent } from '../../../buildtime/utils.js'
...
class Bundle {
    ...
    async apply() {
        ...
        await writeFileIfDifferent(output, [exportStr].join('\n'))
    }
}
...
export const createBundles = async (node, outputDir) => {
    ...
    for (node of nodes) {
        ...
        } else node.module = `() => import('${relative(routifyDir, node.module)}')::_EVAL` //codesplit file
    }
