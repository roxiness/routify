...
export const watcher = ({ instance }) => {
    ...
    if (!instance._chokidar) {
        ...
        watcher.on('change', eventHandler('fileChanged', 'File %s has been changed'))
        watcher.on('unlink', eventHandler('fileRemoved', 'File %s has been removed'))
    }
}
...
const createEventHandler = instance => (hook, msg) => async path => {
    instance.on[hook]
    instance.on[hook].runHooks(path)
