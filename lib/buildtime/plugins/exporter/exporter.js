...
import { createDirname, stringifyWithEscape, writeDynamicImport } from '../../utils.js'
...
export const exportNode = (rootNode, outputDir) => {
    ...
    const imports = [rootNode, ...rootNode.descendants]
        ...
        .map(
            node =>
                `import * as ${node.id} from '${relative(
                    outputDir,
                    node.component,
                ).replace(/\\/g, '/')}'`,
        )
    ...
    ;[rootNode, ...rootNode.descendants]
        .filter(node => node.component && !node.component.match(/import\(/))
        .forEach(node => {
            if (node.meta.split)
