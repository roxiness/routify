...
export const exportNode = (rootNode, outputDir) => {
    ...
    const imports = [rootNode, ...rootNode.descendants]
        .filter(node => node.module && !node.module.startsWith('import('))
        .filter(node => !node.meta.split)
        .map(
            node =>
                `import * as ${node.id} from '${relative(outputDir, node.module).replace(
                    /\\/g,
                    '/',
                )}'`,
        )
    ...
    // set module to either id or import(module).then(r => r.default)
    ;[rootNode, ...rootNode.descendants]
        .filter(node => node.module && !node.module.match(/import\(/))
        .forEach(node => {
            if (node.meta.split)
                node.module = `() => import('${relative(outputDir, node.module).replace(
                    /\\/g,
                    '/',
                )}')`
            else node.module = `() => ${node.id}`
        })
