...
const applyTransforms = async (id, content, transforms) => {
    for (const transform of transforms) content = await transform(id, content)
    return content
}
...
export class RoutifyBuildtime extends Routify {
    ...
    async writeFile(id, content) {
        ...
        const output = await applyTransforms(id, content, transforms)
        await writeFileIfDifferent(id, output)
    }
