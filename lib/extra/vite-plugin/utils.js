...
export const postSsrBuildProcess = async options => {
    ...
    fse.copySync(__dirname + `/assets/${type}`, `${options.outDir}/server`)

    if (type === 'cjs') fse.writeFileSync(`${options.outDir}/server/package.json`, '{}')

    fse.moveSync(
        `${options.outDir}/client/index.html`,
        `${options.outDir}/server/index.html`,
        { overwrite: true },
    )
...
export const normalizeOptions = (input = {}, isProduction) => {
    ...
    return /** @type {VitePluginOptions} */ ({
        ...
        run: input.run ?? true,
        forceLogging: input.forceLogging ?? true,
        render: {
